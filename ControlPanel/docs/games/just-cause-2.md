# Just Cause 2 — Complete Dedicated Server Guide

## Startup Parameters
**Default command line**
```bash
./just-cause-2_server -port 27015 -maxplayers 16 -config server.cfg
```
...
**Windows:**
- `just-cause-2_server.cfg` — Main server configuration
- `config/` — Configuration directory
- `logs/` — Log files directory
- `data/` — Server data and saves

**Linux:**
- `~/just-cause-2/server.cfg` — Main server configuration  
- `~/just-cause-2/config/` — Configuration directory
- `~/just-cause-2/logs/` — Log files directory
- `~/just-cause-2/data/` — Server data and saves
...
- Check official mod repositories or community sites for Just Cause 2
