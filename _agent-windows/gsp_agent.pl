...
sub get_log
{
	...
	{
		# logger "DEBUG DIRECTORYNAME IS ". $home_path;
		# logger "DEBUG LOGFILE IS ". $log_file;
        
			
		 $log_file = Path::Class::File->new($home_path, $log_file);
		 # logger "debug NEW logfile is ".$log_file;
		 # my @timestamplog = `ls $log_file -t`;
		 # logger "DEBUG timestamplog is ".$timestamplog[1];
		 # $log_file = $timestamplog[1];
	...
	if(SCREEN_LOG_LOCAL == 1)
	{
		...
		# logger "DEBUG DIRECTORY IS ". $directoryname;
		# my $timestamplog = `ls $log_file -t`;
		# logger "DEBUG LOGFILE IS ". $log_file;
        #logger "DEGUB timestampfile is ". $timestamplog;
		# $log_file = $timestamplog;
