<?php
/**
 * Garrys Mod Server Documentation - Comprehensive Guide
...
<h1>Garrys Mod Server Hosting Guide</h1>

<h2>Overview</h2>
<p>Garrys Mod is a multiplayer game server that can be hosted on a VPS or dedicated server. This comprehensive guide covers everything you need to know about hosting a Garrys Mod server for your community.</p>
...
<div style="background: #1e3a5f; padding: 20px; border-left: 4px solid #3b82f6; margin: 20px 0; border-radius: 4px;">
    <ul style="color: #e5e7eb; line-height: 1.8; margin: 0;">
        ...
        <li><strong style="color: #ffffff;">Steam App ID:</strong> <code style="background: #0f172a; padding: 2px 6px; border-radius: 3px; color: #a5b4fc;">4020</code></li>
...
<div style="background: #1e3a5f; padding: 20px; border-left: 4px solid #3b82f6; margin: 20px 0; border-radius: 4px;">
    ...
    <p style="color: #e5e7eb;">The Garrys Mod server typically uses a configurable port. Check your server configuration files for the specific port settings.</p>
...
# Windows Firewall
netsh advfirewall firewall add rule name="Garrys Mod Server" dir=in action=allow protocol=TCP localport=[PORT]
netsh advfirewall firewall add rule name="Garrys Mod Server" dir=in action=allow protocol=UDP localport=[PORT]
</code></pre>
...
<p><strong>This game can be installed via SteamCMD using App ID: 4020</strong></p>
...
mkdir -p ~/gameservers/garrysmod

# Run SteamCMD and download
steamcmd +login anonymous \
         +force_install_dir ~/gameservers/garrysmod \
         +app_update 4020 validate \
         +quit

# Server files are now in ~/gameservers/garrysmod/
cd ~/gameservers/garrysmod
ls -la
</code></pre>
...
steamcmd.exe +login anonymous ^
             +force_install_dir C:\gameservers\garrysmod ^
             +app_update 4020 validate ^
             +quit
</code></pre>
...
<pre><code>-console %GAME_TYPE% %HOSTNAME% %PID_FILE% %MAP% %IP% %PORT% %PLAYERS% -condebug</code></pre>
...
<div style="background: #1e3a5f; padding: 20px; border-left: 4px solid #3b82f6; margin: 20px 0; border-radius: 4px;">
    ...
    <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #374151;">
        <h4 style="color: #ffffff; margin-top: 0;">
            <code style="background: #0f172a; padding: 4px 8px; border-radius: 3px; color: #a5b4fc;">+map</code>
            <span style="color: #e5e7eb; font-weight: normal; font-size: 0.9em;"> - Map</span>
        </h4>
        <p style="color: #e5e7eb; margin: 10px 0;">Set the map the server will start.</p>
        <p style="color: #fbbf24;"><strong>Default:</strong> <code style="background: #0f172a; padding: 2px 6px; border-radius: 3px;">gm_flatgrass</code></p>
    </div>

    <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #374151;">
        <h4 style="color: #ffffff; margin-top: 0;">
            <code style="background: #0f172a; padding: 4px 8px; border-radius: 3px; color: #a5b4fc;">+gamemode</code>
            <span style="color: #e5e7eb; font-weight: normal; font-size: 0.9em;"> - Gamemode</span>
        </h4>
        <p style="color: #e5e7eb; margin: 10px 0;">Set the gamemode the server will use.</p>
    </div>
    ...
    <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #374151;">
        <h4 style="color: #ffffff; margin-top: 0;">
            <code style="background: #0f172a; padding: 4px 8px; border-radius: 3px; color: #a5b4fc;">+host_workshop_collection</code>
            <span style="color: #e5e7eb; font-weight: normal; font-size: 0.9em;"> - Workshop Collection</span>
        </h4>
        <p style="color: #e5e7eb; margin: 10px 0;">Set the Workshop collection id. For help, go to http://wiki.garrysmod.com/page/Workshop_for_Dedicated_Servers </p>
    </div>

    <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #374151;">
        <h4 style="color: #ffffff; margin-top: 0;">
            <code style="background: #0f172a; padding: 4px 8px; border-radius: 3px; color: #a5b4fc;">-authkey</code>
            <span style="color: #e5e7eb; font-weight: normal; font-size: 0.9em;"> - Steam API Key</span>
        </h4>
        <p style="color: #e5e7eb; margin: 10px 0;">Set the Steam API key for workshop usage, you can get one by going to: http://steamcommunity.com/dev/apikey</p>
    </div>
    ...
    <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #374151;">
        <h4 style="color: #ffffff; margin-top: 0;">
            <code style="background: #0f172a; padding: 4px 8px; border-radius: 3px; color: #a5b4fc;">+sv_loadingurl</code>
            <span style="color: #e5e7eb; font-weight: normal; font-size: 0.9em;"> - Loading URL</span>
        </h4>
        <p style="color: #e5e7eb; margin: 10px 0;">URL to show to clients while joining the server. Please remove http:// from URL or it will not work.</p>
    </div>
...
<ul>
    <li>Official Garrys Mod documentation and forums</li>
...
<p style="text-align: center; margin-top: 30px; color: #666;">
    <em>Last updated: November 2025 | For Garrys Mod server hosting</em>
</p>
