<?php
/**
 * Starbound Server Documentation - Comprehensive Guide
...
<h1>Starbound Server Hosting Guide</h1>

<h2>Overview</h2>
<p>Starbound is a multiplayer game server that can be hosted on a VPS or dedicated server. This comprehensive guide covers everything you need to know about hosting a Starbound server for your community.</p>

<h2 id="quick-info">Quick Info</h2>
<div style="background: #1e3a5f; padding: 20px; border-left: 4px solid #3b82f6; margin: 20px 0; border-radius: 4px;">
    <ul style="color: #e5e7eb; line-height: 1.8; margin: 0;">
        <li><strong style="color: #ffffff;">Default Port:</strong> <code style="background: #0f172a; padding: 2px 6px; border-radius: 3px; color: #a5b4fc;">21025</code></li>
        ...
        <li><strong style="color: #ffffff;">Steam App ID:</strong> <code style="background: #0f172a; padding: 2px 6px; border-radius: 3px; color: #a5b4fc;">533830</code></li>
        <li><strong style="color: #ffffff;">Recommended OS:</strong> Linux (Ubuntu/Debian) or Windows Server</li>
            ...
            <li><code style="background: #0f172a; padding: 2px 6px; border-radius: 3px; color: #a5b4fc;">storage/starbound_server.config</code> - Server Configurations</li>
        </ul></li>
    </ul>
</div>
...
<div style="background: #1e3a5f; padding: 20px; border-left: 4px solid #3b82f6; margin: 20px 0; border-radius: 4px;">
    ...
    <p style="color: #e5e7eb;">The Starbound server typically uses a configurable port. Check your server configuration files for the specific port settings.</p>
...
# Windows Firewall
netsh advfirewall firewall add rule name="Starbound Server" dir=in action=allow protocol=TCP localport=[PORT]
netsh advfirewall firewall add rule name="Starbound Server" dir=in action=allow protocol=UDP localport=[PORT]
</code></pre>
...
<p><strong>This game can be installed via SteamCMD using App ID: 533830</strong></p>
...
mkdir -p ~/gameservers/starbound

# Run SteamCMD and download
steamcmd +login anonymous \
         +force_install_dir ~/gameservers/starbound \
         +app_update 533830 validate \
         +quit

# Server files are now in ~/gameservers/starbound/
cd ~/gameservers/starbound
ls -la
</code></pre>
...
steamcmd.exe +login anonymous ^
             +force_install_dir C:\gameservers\starbound ^
             +app_update 533830 validate ^
             +quit
</code></pre>
...
<ul>
    <li><strong><code>storage/starbound_server.config</code></strong> - Server Configurations</li>
</ul>
