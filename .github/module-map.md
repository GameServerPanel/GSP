...
| `steam_workshop` | `modules/steam_workshop/*` (deprecated UI routes), `Panel/includes/functions.php` | Legacy standalone module kept only for compatibility helpers/data. Direct page routes are disabled and monitor entry now forwards to Server Content Manager (`addonsmanager`). | Uses `config_games` XML metadata + `server_homes`/assignment tables; SCM is the supported Workshop user/admin path. |
